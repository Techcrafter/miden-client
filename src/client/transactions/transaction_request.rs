...
use miden_objects::{
    accounts::AccountId, assets::{Asset, FungibleAsset}, notes::{Note, NoteDetails, NoteId, NoteType, PartialNote}, transaction::{TransactionArgs, TransactionScript}, vm::AdviceMap, Digest, Hasher, Word
};
...
impl ExpectedNote {
    pub fn id(&self) -> NoteId {
        match self {
            ExpectedNote::Full(n) => n.id(),
            ExpectedNote::Partial(n) => n.id(),
        }
    }
    ...
    pub fn authentication_hash(&self) -> Digest {
        match self {
            ...
            ExpectedNote::Partial(partial_note) => {
                // TODO: This should go into miden-objects
                Hasher::merge(&[partial_note.id().inner(), Word::from(partial_note.metadata()).into()])
            },
        }
    }
}
...
pub struct TransactionRequest {
    ...
    expected_output_notes: Vec<ExpectedNote>,
...
impl TransactionRequest {
    ...
    pub fn expected_output_notes(&self) -> &[ExpectedNote] {
    ...
    pub fn future_note_details(&self) -> &[NoteDetails] {
        &self.future_note_details
    }
}
...
impl From<TransactionRequest> for TransactionArgs {
    fn from(val: TransactionRequest) -> Self {
        ...
        let output_notes = val.expected_output_notes
        .into_iter() 
        .filter_map(|expected_note| {
            match expected_note {
                ExpectedNote::Full(note) => Some(note),
                // TODO: We need to update the transaction args for partial notes but we don't 
                // have a mechanism to do so right now.
                ExpectedNote::Partial(_) => None,
            }
        });
